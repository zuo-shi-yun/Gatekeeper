...
class HandleRequest:
    ...
    def is_tourist_out_usage(self) -> bool:
        ...
        if self.sender_id in qq_list:  # 在临时用户数据库中
            use_time = svc.query(['time'], {'qq': self.sender_id})[0]  # 上次使用时间
            if self.is_last_use_time_out_refresh_time(self.cfg['tourist_refresh_days'], use_time):  # 若上次使用已过刷新时间范围
                ...
                # 更新数据库
                svc.update({'time': formatted_time, 'usage': 1, 'max_usage': max_usage}, {'qq': self.sender_id})
                return True
            else:  # 上次使用未过刷新时间范围
                use_time = svc.query(['usage'], {'qq': self.sender_id})[0]
                ...
                if use_time >= max_use_time:  # 已经超出则禁止使用
                    ...
                    svc.update({'usage': use_time + 1}, {'qq': self.sender_id})
                    return True
        else:  # 没有在则插入并设次数为1
            if self.cfg['tourist_random_usage']:  # 开启了随机配额:
                max_usage = random.randint(self.cfg['tourist_min_usage'], self.cfg['tourist_max_usage'])
            ...
            # 插入数据库
            svc.insert(
                {'qq': self.sender_id, 'time': datetime.datetime.now().strftime("%m-%d"), 'usage': 1,
                 'max_usage': max_usage})
            return True
